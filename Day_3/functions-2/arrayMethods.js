...
const postTaxPrices = prices.map((element, index, array)=>element * 1.07);
...
const myStrongest = monstersInYourPocket.filter((element, index, array) => array[index].CP > 200);
...
const ordersTaxed = orders.map((element, index, array)=>array[index].price * (1 + array[index].tax));
...
const bobsPurchases = purchases.filter((element, index, wholeArray)=> wholeArray[index].owner === 'Bob');
console.log(bobsPurchases);
const bobsTotal = bobsPurchases.reduce((previousValue, currentValue)=>previousValue + currentValue.price, 0);
